...
export function GlobalOperationFeedback({
  ...
  if (!feedback && pendingClipboardRead) {
    ...
    feedback = {
      message: textTransferResult.message,
      tone: textTransferResult.succeeded ? "success" : "error",
    };
    ...
    feedback = {
      message: appLaunchResult.message,
      tone: appLaunchResult.succeeded ? "success" : "error",
    };
  ...
  return feedback ? (
    <AppToast
      tone={feedback.tone}
      {...(transientFeedback && onDismissTransient ? { onDismiss: onDismissTransient } : {})}
    >
      {feedback.message}
    </AppToast>
  ) : null;
...
function presentationActionLabel(action: PresentationCommandResultMessage["action"]): string {
  switch (action) {
    case "activate":
      return "Bring PowerPoint forward";
    case "start":
      return "Start from beginning";
    case "start-current":
      return "Start from current";
    case "next":
      return "Next";
    case "previous":
      return "Previous";
