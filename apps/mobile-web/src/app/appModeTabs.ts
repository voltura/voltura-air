...
export const toolModeDefinitions: Record<ToolAppTab, ModeDefinition> = {
  ...
  "text-transfer": {
    id: "text-transfer",
    label: "Send text",
    ariaLabel: "Send text to PC",
    Icon: Send,
  },
  "clipboard-read": {
    id: "clipboard-read",
    label: "Get text",
    ariaLabel: "Get text from PC",
    Icon: ClipboardPaste,
  },
...
export function getModeDefinition(tab: MainAppTab): ModeDefinition {
  return (
    primaryModeDefinitions.find((mode) => mode.id === tab) ??
    toolModeDefinitions[tab as ToolAppTab] ??
    toolModeDefinitions.dictation
  );
}
