...
describe("CustomScreenBrowserPreview", () => {
  ...
  it("accepts only bounded opaque custom-screen IDs", () => {
    expect(
      readCustomScreenPreviewId("http://127.0.0.1/?customScreenPreview=screen.preview-1"),
    ).toBe("screen.preview-1");
    expect(
      readCustomScreenPreviewId("http://127.0.0.1/?customScreenPreview=../../private"),
    ).toBeNull();
  });
  ...
  it("loads the sanitized preview definition from the loopback API", async () => {
    ...
    const view = render(<CustomScreenBrowserPreview controlDepth screenId="screen.preview" />);

    await waitFor(() => {
      expect(view.container.querySelector(".custom-screen-workspace")).not.toBeNull();
      expect(
        view.container.querySelector(".custom-screen-workspace")?.getAttribute("aria-label"),
      ).toBe("Preview screen");
    });
    ...
    const previewRoot = view.container.querySelector(".custom-screen-browser-preview");
    expect(previewRoot?.classList.contains("control-depth")).toBe(true);
    expect(previewRoot?.firstElementChild?.classList.contains("custom-screen-workspace")).toBe(
      true,
    );
  });
});
