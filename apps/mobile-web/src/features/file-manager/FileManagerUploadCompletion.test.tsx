...
vi.mock("./FileTransferMenu", () => ({
  FileTransferMenu: ({
    ...
    <button type="button" onClick={() => onUploadCompleted?.("left", "uploaded.txt")}>
...
describe("FileManagerWorkspace upload completion", () => {
  ...
  it("refreshes the destination and selects the host-confirmed uploaded file", () => {
    ...
    if (refresh?.type !== "file.refresh") {
      throw new Error("Expected the uploaded folder to refresh.");
    }
    act(() =>
      publishFileManagerResult({
        ...
        page: page(["existing.txt", "uploaded.txt"]),
      }),
    );

    expect(
      (screen.getByRole("checkbox", { name: "Select uploaded.txt" }) as HTMLInputElement).checked,
    ).toBe(true);
    expect(
      (screen.getByRole("checkbox", { name: "Select existing.txt" }) as HTMLInputElement).checked,
    ).toBe(false);
  });
  ...
  it("continues paging until the uploaded file can be selected", () => {
    ...
    act(() =>
      publishFileManagerResult({
        ...
        succeeded: true,
        message: "Folder loaded.",
        page: page(firstPageNames, "next-page", 101),
      }),
    );
    const nextPage = [...sent].reverse().find((message) => message.type === "file.page.get");
    if (nextPage?.type !== "file.page.get") {
      throw new Error("Expected Files to locate the uploaded row in the next page.");
    }
    act(() =>
      publishFileManagerResult({
        type: "file.page.get.result",
        operationId: nextPage.operationId,
        succeeded: true,
        message: "Folder loaded.",
        page: page(["uploaded.txt"], null, 101),
      }),
    );
