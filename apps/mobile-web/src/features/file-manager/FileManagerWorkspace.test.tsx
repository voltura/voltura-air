...
describe("FileManagerWorkspace pagination and selection", () => {
  ...
  it("uses Copy for the other panel in a wide layout and dismisses transient menus on other interactions", () => {
    ...
    expect([...sent].reverse().find((message) => message.type === "file.job.create")).toMatchObject({
      type: "file.job.create", operation: "copy", panel: "left", destinationPanel: "right", destinationRevision: "right-revision", entryIds: ["entry-0"]
    });
  ...
  it("enters PC Screen only after View opens the selected file successfully", () => {
    ...
    if (failedOpen?.type !== "file.open") {throw new Error("Expected View to open the file.");}
    expect(onMirrorView).not.toHaveBeenCalled();
    act(() => publishFileManagerResult({ type: "file.open.result", operationId: failedOpen.operationId, succeeded: false, code: "shell-failed", message: "Windows could not open this file." }));
    expect(onMirrorView).not.toHaveBeenCalled();
    ...
    if (opened?.type !== "file.open") {throw new Error("Expected View to retry opening the file.");}
    act(() => publishFileManagerResult({ type: "file.open.result", operationId: opened.operationId, succeeded: true, message: "Opened." }));
  ...
  it("keeps a failed operation selected and clears a completed operation selection", () => {
    ...
    if (failedRequest?.type !== "file.job.create") {throw new Error("Expected a copy job.");}
    act(() => publishFileManagerResult({ type: "file.job.create.result", operationId: failedRequest.operationId, succeeded: true, message: "Queued.", job: job("job-failed", "copy", "running") }));
    act(() => publishFileManagerResult({ type: "file.jobs.status", jobs: [job("job-failed", "copy", "failed")] }));
    expect((checkbox as HTMLInputElement).checked).toBe(true);
    expect(sent.some((message) => message.type === "file.refresh" && message.panel === "right")).toBe(true);
    expect(sent.some((message) => message.type === "file.refresh" && message.panel === "left")).toBe(false);

    fireEvent.click(screen.getByRole("button", { name: "Copy" }));
    const completedRequest = [...sent].reverse().find((message) => message.type === "file.job.create");
    if (completedRequest?.type !== "file.job.create") {throw new Error("Expected a second copy job.");}
    act(() => publishFileManagerResult({ type: "file.job.create.result", operationId: completedRequest.operationId, succeeded: true, message: "Queued.", job: job("job-complete", "copy", "running") }));
    act(() => publishFileManagerResult({ type: "file.jobs.status", jobs: [job("job-failed", "copy", "failed"), job("job-complete", "copy", "completed")] }));
  ...
  it("keeps terminal operation history reachable and dismissible", () => {
    ...
    expect([...sent].reverse().find((message) => message.type === "file.job.control")).toMatchObject({
      type: "file.job.control", jobId: "job-failed", action: "dismiss"
    });
  });
});

function renderWorkspace(sent: ClientMessage[], onMirrorView = vi.fn()) {
  render(<FileManagerWorkspace capability={{ canBrowse: true, canModify: true, hidesProtectedSystemItems: true, maxPageSize: 100 }} canMirrorView connectionEpoch={1} mirrorViewUnavailableMessage="PC Screen unavailable." onMirrorView={onMirrorView} send={(message) => sent.push(message)} state="paired" />);
}

function openSession(sent: ClientMessage[], leftEntries: FileManagerEntry[], rightEntries: FileManagerEntry[]) {
  const open = [...sent].reverse().find((message) => message.type === "file.session.open");
  if (open?.type !== "file.session.open") {throw new Error("Expected Files to open a session.");}
  const panelPage = (panel: "left" | "right", panelEntries: FileManagerEntry[]): FileManagerPanelPage => ({
    panel, revision: `${panel}-revision`, displayPath: panel === "left" ? "Downloads" : "Documents", parentId: `${panel}-parent`,
    driveId: "drive-a", sortBy: "name", descending: false, totalCount: panelEntries.length, entries: panelEntries, continuation: null
  });
  act(() => publishFileManagerResult({
    type: "file.session.open.result", operationId: open.operationId, succeeded: true, message: "Opened.",
    session: { sessionId: "session-a", drives: [{ id: "drive-a", label: "C:", driveType: "fixed" }], shortcuts: [{ id: "downloads-a", label: "Downloads" }], left: panelPage("left", leftEntries), right: panelPage("right", rightEntries) }
  }));
}

function job(jobId: string, operation: "copy" | "move" | "paste" | "rename" | "delete", state: "running" | "completed" | "failed") {
  return { jobId, operation, state, queuePosition: 0, itemsCompleted: state === "completed" ? 1 : 0, itemsTotal: 1, bytesCompleted: state === "completed" ? 12 : 0, bytesTotal: 12, message: state === "failed" ? "Copy failed." : "Working.", canPause: state === "running", canResume: false, canCancel: state === "running" } as const;
}
