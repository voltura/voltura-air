...
import {
  createFileTransferAcknowledgement, createFileTransferDataRecord, maximumFileTransferPayloadBytes, parseFileTransferRecord
...
describe("file transfer records", () => {
  it("round-trips data and zero-byte cumulative acknowledgements with big-endian offsets", () => {
    ...
    expect(parseFileTransferRecord(data)).toEqual({ kind: "data", offset: 0x01020304050607, payload: new Uint8Array([1, 2, 3]) });
    expect(parseFileTransferRecord(acknowledgement)).toEqual({ kind: "acknowledgement", offset: 0, payload: new Uint8Array() });
  });
  ...
  it("rejects invalid versions, shapes, sizes, and unsafe offsets", () => {
    ...
    expect(parseFileTransferRecord(new Uint8Array([0x11, 0, 0, 0, 0, 0, 0, 0, 0]).buffer)).toBeNull();
    expect(parseFileTransferRecord(new Uint8Array([0x12, 0, 0, 0, 0, 0, 0, 0, 0, 1]).buffer)).toBeNull();
    ...
    expect(() => createFileTransferDataRecord(0, new Uint8Array(maximumFileTransferPayloadBytes + 1))).toThrow(RangeError);
  });
});
