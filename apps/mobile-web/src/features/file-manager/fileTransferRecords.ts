...
export function createFileTransferDataRecord(offset: number, payload: Uint8Array): ArrayBuffer {
  if (!Number.isSafeInteger(offset) || offset < 0 || payload.byteLength < 1 || payload.byteLength > maximumFileTransferPayloadBytes) {
...
export function createFileTransferAcknowledgement(offset: number): ArrayBuffer {
  if (!Number.isSafeInteger(offset) || offset < 0) {throw new RangeError("Invalid file-transfer acknowledgement.");}
...
export function parseFileTransferRecord(value: unknown): FileTransferRecord | null {
  ...
  if (!bytes || header === undefined || bytes.byteLength < headerBytes || bytes.byteLength > headerBytes + maximumFileTransferPayloadBytes || header >> 4 !== version) {return null;}
  const offsetValue = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength).getBigUint64(1, false);
  if (offsetValue > BigInt(Number.MAX_SAFE_INTEGER)) {return null;}
  const kind = header & 0x0f;
  const payload = bytes.slice(headerBytes);
  if (kind === dataKind && payload.byteLength > 0) {return { kind: "data", offset: Number(offsetValue), payload };}
  if (kind === acknowledgementKind && payload.byteLength === 0) {return { kind: "acknowledgement", offset: Number(offsetValue), payload };}
  return null;
}
