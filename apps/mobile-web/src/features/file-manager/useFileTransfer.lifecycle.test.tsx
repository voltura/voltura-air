...
const storage = vi.hoisted(() => {
  ...
  const writable = { abort: vi.fn(() => Promise.resolve()), close: vi.fn(() => Promise.resolve()), write: vi.fn(() => Promise.resolve()) };
  ...
  return {
    ...
    save: vi.fn<() => Promise<"shared" | "download-started">>(() => Promise.resolve("download-started"))
  };
});

vi.mock("../../foundation/connection/pairingCredentials", () => ({ signClientPayload: () => "client-signature" }));
vi.mock("../../foundation/webrtc/iceGathering", () => ({ hasOnlyRelayCandidates: () => true, waitForIceGathering: () => Promise.resolve() }));
vi.mock("../../foundation/webrtc/sessionCrypto", () => ({ hashSessionDescription: () => "sdp-hash", verifyHostSessionSignature: () => true }));
...
class TestPeerConnection {
  ...
  private readonly listeners = new Map<string, EventListener[]>();
  constructor() {TestPeerConnection.latest = this;}
  addEventListener(type: string, listener: EventListener) {
    this.listeners.set(type, [...(this.listeners.get(type) ?? []), listener]);
  }
  setRemoteDescription() {
    this.listeners.get("datachannel")?.forEach((listener) => listener({ channel: this.channel } as unknown as Event));
...
describe("useFileTransfer download lifecycle", () => {
  ...
  it("cancels a pending start by request ID before the host issues a transfer ID", () => {
    ...
    const target: FileTransferTarget = {
      sessionId: "session", panel: "left", revision: "revision",
      entry: { id: "file-a", name: "report.txt", kind: "file", extension: "txt", size: 1, modifiedUtc: "2026-08-25T00:00:00Z", attributes: [] }
    };
    ...
    function Harness() {
      transfer = useFileTransfer(
        { customName: false, id: "pc", name: "PC", url: "https://pc.invalid", hostIdentityPublicKey: "host-key", transportMode: "secure-direct" },
        "client", true, send
      );
      return null;
    }
    ...
    if (start?.type !== "file.transfer.start") {throw new Error("Expected a transfer start.");}
    expect(sent.some((message) => message.type === "file.transfer.cancel")).toBe(false);
    act(() => transfer!.cancel());

    expect(sent.at(-1)).toMatchObject({ type: "file.transfer.cancel", requestId: start.operationId });
  ...
  it("publishes transfer failures to the persistent Files status", () => {
    ...
    function Harness() {
      transfer = useFileTransfer(
        { customName: false, id: "pc", name: "PC", url: "https://pc.invalid", hostIdentityPublicKey: "host-key", transportMode: "secure-direct" },
        "client", true, send, undefined, notice
      );
      return null;
    }
    render(<Harness />);
    ...
    act(() => publishFileManagerResult({
      type: "file.transfer.start.result", operationId: start.operationId, succeeded: false,
      code: "file-unavailable", message: "The selected file is unavailable."
    }));
  ...
  it("keeps a canceled iPhone share ready, then clears it after a successful handoff", async () => {
    ...
    const target: FileTransferTarget = {
      sessionId: "session", panel: "left", revision: "revision",
      entry: { id: "file-a", name: "report.txt", kind: "file", extension: "txt", size: 0, modifiedUtc: "2026-08-25T00:00:00Z", attributes: [] }
    };
    let transfer: ReturnType<typeof useFileTransfer> | null = null;
    function Harness() {
      transfer = useFileTransfer(
      ...
      return <span>{transfer.presentation.readyToSave ? "ready" : transfer.presentation.message}</span>;
    }
    render(<Harness />);
    ...
    const start = sent.find((message) => message.type === "file.transfer.start");
    if (start?.type !== "file.transfer.start") {throw new Error("Expected a transfer start.");}
    act(() => publishFileManagerResult({ type: "file.transfer.start.result", operationId: start.operationId, succeeded: true, message: "Started.", transferId: "transfer-a" }));
    act(() => publishFileManagerResult({
      type: "file.transfer.offer", transferId: "transfer-a", direction: "download", fileName: "report.txt", declaredSize: 0,
      offerSdp: "offer-sdp", hostSignature: "host-signature"
    }));
    await waitFor(() => expect(sent.some((message) => message.type === "file.transfer.answer")).toBe(true));
    act(() => publishFileManagerResult({
      type: "file.transfer.result", transferId: "transfer-a", direction: "download", succeeded: true,
      message: "File ready to save.", fileName: "report.txt", declaredSize: 0
    }));
    ...
    act(() => transfer!.startDownload({ ...target, entry: { ...target.entry!, id: "file-b", name: "next.txt" } }));
  ...
  it("removes storage that finishes preparing after Files has closed", async () => {
    ...
    storage.prepare.mockReturnValueOnce(new Promise((resolve) => {releaseStorage = resolve;}));
    const sent: ClientMessage[] = [];
    const send = (message: ClientMessage) => sent.push(message);
    const target: FileTransferTarget = {
      sessionId: "session", panel: "left", revision: "revision",
    ...
    rendered.unmount();
    act(() => {releaseStorage!(storage.prepared);});
