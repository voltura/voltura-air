...
describe("ThirdPartyNoticesWorkspace", () => {
  ...
  it("parses the generated notice sections into app content", () => {
    expect(parseThirdPartyNotices(noticeSource)).toEqual([
      { name: "alpha 1.0.0", license: "MIT", source: "https://example.com/alpha", text: "Alpha license text." },
      { name: "beta 2.0.0", license: "Apache-2.0", source: "https://example.com/beta", text: "Beta license text." }
    ]);
  });
  ...
  it("loads notices as a navigable workspace", async () => {
    ...
    expect(screen.getAllByRole("link", { name: /Source/ })[0]?.getAttribute("href")).toBe("https://example.com/alpha");
