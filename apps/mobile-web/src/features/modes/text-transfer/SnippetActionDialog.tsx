...
export function SnippetActionDialog({
  action,
  nameTaken,
  onCancel,
  onConfirm,
}: SnippetActionDialogProps) {
  const nameInputRef = useRef<HTMLInputElement>(null);
  const [name, setName] = useState(action.snippet.name);
  const normalizedName = normalizeSnippetName(name);
  const isNameTaken =
    action.kind === "rename" && normalizedName.length > 0 && nameTaken(normalizedName);
  const title =
    action.kind === "rename"
      ? "Rename snippet"
      : action.kind === "update"
      ...
      actions={
        <>
          ...
          <button
            type="submit"
            className={
              action.kind === "delete" ? "snippet-dialog-danger" : "snippet-dialog-primary"
            }
    ...
    >
      <p>
        {action.kind === "rename" ? (
          ...
          <>
            Permanently delete <strong>{action.snippet.name}</strong> from this device?
          </>
        )}
      </p>
      ...
      {action.kind === "rename" && (
        <label>
          ...
          />
          {isNameTaken && (
            <span id="rename-snippet-name-error" className="snippet-name-error" role="alert">
