import type { KeyboardSpecialMessage, KeyboardTextMessage } from "../../foundation/protocol/messages";
...
export function screenKeyboardMessage(event: Pick<KeyboardEvent,
  "altKey" | "code" | "ctrlKey" | "getModifierState" | "isComposing" | "key" | "metaKey" | "shiftKey"
>): ScreenKeyboardMessage | null {
  if (event.isComposing || event.key === "Dead" || event.key === "Process" || event.key === "Unidentified" || isModifierKey(event.key)) {
    return null;
  }
...
function screenSpecialKey(key: string, code: string) {
  if (key === " ") {return "Space";}
  if (code.startsWith("Numpad") && supportedSpecialKeys.has(code)) {return code;}
