...
describe("screenViewQualityFromStats", () => {
  it("reports exact inbound dimensions, frame rate, and interval bitrate", () => {
    ...
    const result = screenViewQualityFromStats(
      report,
      null,
      { bytesReceived: 1_000_000, sampledAt: 1_000, framesDecoded: 0, framesDropped: 0, freezeCount: 0, packetsLost: 0 },
      2_000);
  ...
  it("ignores reports without an inbound video stream", () => {
    const report = new Map([["audio", { id: "audio", timestamp: 0, type: "inbound-rtp", kind: "audio" }]]) as unknown as RTCStatsReport;
...
describe("startScreenViewQualityMonitor", () => {
  it("keeps at most one browser statistics request in flight", async () => {
    ...
    const getStats = vi.fn()
      .mockImplementationOnce(() => new Promise<RTCStatsReport>((resolve) => {resolveFirst = resolve;}))
      .mockResolvedValue(new Map() as unknown as RTCStatsReport);
    const onReport = vi.fn();
    const stop = startScreenViewQualityMonitor({ getStats } as Pick<RTCPeerConnection, "getStats">, onReport);
  ...
  it("does not publish or reschedule an in-flight sample after cleanup", async () => {
    ...
    const getStats = vi.fn(() => new Promise<RTCStatsReport>((complete) => {resolve = complete;}));
    const onReport = vi.fn();
    const stop = startScreenViewQualityMonitor({ getStats } as Pick<RTCPeerConnection, "getStats">, onReport);
