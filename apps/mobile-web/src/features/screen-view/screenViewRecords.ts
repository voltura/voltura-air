...
export interface ScreenStatusRecord {
  type: "status";
  code: string;
  message: string;
}
...
export function parseScreenPlaintextRecord(bytes: Uint8Array): ScreenPlaintextRecord {
  if (bytes.length === 0 || bytes.length > maxPlaintextBytes) {
    throw new Error("Invalid screen event size.");
  }
  if (bytes[0] === 4) {
    return parseCursor(bytes);
  }
  if (bytes[0] === 5) {
...
function parseCursor(bytes: Uint8Array): ScreenCursorRecord {
  if (bytes.length < 39) {
    throw new Error("Truncated cursor record.");
  }
  const view = dataView(bytes);
  const visibleByte = bytes[9];
  if (visibleByte !== 0 && visibleByte !== 1) {
    throw new Error("Invalid cursor visibility.");
  }
  ...
  if (
    length < 0 ||
    length > maxCursorBytes ||
    length !== bytes.length - 39 ||
    (length === 0 ? format !== 0 : format !== 2)
  ) {
...
function parseStatus(bytes: Uint8Array): ScreenStatusRecord {
  if (bytes.length < 4) {
    throw new Error("Truncated status record.");
  }
  ...
  if (
    ...
    messageLength === 0 ||
    messageLength > 512 ||
    4 + codeLength + messageLength !== bytes.length
  ) {
