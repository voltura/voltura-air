...
export function parseFileTransferServerMessage(data: unknown): FileTransferServerMessage | null {
  ...
  if (value.type === "file.transfer.offer") {
    return exact(frame, [
      ...
      transfer(frame) &&
      text(frame.fileName, 255) &&
      size(frame.declaredSize) &&
      text(frame.offerSdp, 32 * 1024) &&
      text(frame.hostSignature, 128) &&
      optional(frame, "iceServers", ice) &&
      optional(frame, "turnExpiresAt", (item) => text(item, 40)) &&
      optional(frame, "relayUsageBytes", size) &&
      optional(frame, "relayUsageCheckedAt", (item) => text(item, 40))
      ? accepted(value)
      : null;
  }
  if (value.type === "file.transfer.status") {
    return exact(frame, [
      ...
      transfer(frame) &&
      ["queued", "connecting", "transferring"].includes(String(frame.state)) &&
      size(frame.bytesCompleted) &&
      size(frame.bytesTotal) &&
      frame.bytesCompleted <= frame.bytesTotal
      ? accepted(value)
      : null;
  }
  if (value.type === "file.transfer.result") {
    return exact(frame, [
      ...
      text(frame.fileName, 255) &&
      size(frame.declaredSize) &&
      optional(frame, "jobId", id)
      ? accepted(value)
      : null;
  }
  if (value.type === "file.transfer.start.result") {
    if (
      !exact(frame, ["type", "operationId", "succeeded", "code", "message", "transferId", "job"]) ||
      !result(frame) ||
      !id(frame.operationId) ||
      !optional(frame, "transferId", id) ||
      !optional(frame, "job", fileJob)
    ) {
      return null;
    }
    const succeeded = frame.succeeded === true;
    return succeeded === id(frame.transferId) &&
      (succeeded || !("job" in frame) || frame.job === null)
      ? accepted(value)
      : null;
  }
  if (
    value.type === "file.transfer.answer.result" ||
    value.type === "file.transfer.cancel.result"
  ) {
    return exact(frame, ["type", "operationId", "succeeded", "code", "message"]) &&
      result(frame) &&
      id(frame.operationId)
      ? accepted(value)
      : null;
...
function accepted(value: unknown): FileTransferServerMessage {
  return value as FileTransferServerMessage;
}
...
function transfer(value: Record<string, unknown>): boolean {
  return id(value.transferId) && (value.direction === "download" || value.direction === "upload");
}
...
function fileJob(value: unknown): boolean {
  return (
    record(value) &&
    text(value.jobId, 512) &&
    ["copy", "move", "paste", "delete", "rename", "upload"].includes(String(value.operation)) &&
    [
      "queued",
      "preparing",
      "running",
    ...
    ["queuePosition", "itemsCompleted", "itemsTotal", "bytesCompleted", "bytesTotal"].every(
      (name) => typeof value[name] === "number" && Number.isFinite(value[name]) && value[name] >= 0,
    ) &&
  ...
  return (
    ...
    value.every(
      (server) =>
        ...
        server.urls.every((url) => {
          if (typeof url !== "string" || url.length > 512) {
            return false;
          }
          const match =
            /^(?:turn|turns):[A-Za-z0-9.-]+(?::([0-9]{1,5}))?(?:\?transport=(?:tcp|udp))?$/u.exec(
              url,
            );
          if (!match) {
            return false;
