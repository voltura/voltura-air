...
export function parseRelayKeyChallenge(value: unknown): RelayKeyChallenge | null {
  if (!isRecord(value) || value.type !== "session.key.challenge" || Object.keys(value).length !== 5 ||
      typeof value.routeId !== "string" || !/^[A-Za-z0-9_-]{22}$/u.test(value.routeId) ||
      typeof value.clientId !== "string" || value.clientId.length === 0 || value.clientId.length > 128 ||
      typeof value.hostEphemeralPublicKey !== "string" || !/^[A-Za-z0-9_-]{87}$/u.test(value.hostEphemeralPublicKey) ||
      typeof value.nonce !== "string" || !/^[A-Za-z0-9_-]{43}$/u.test(value.nonce)) {return null;}
...
export function beginRelaySession(
  ...
  try {
    ...
    const shared = p256.getSharedSecret(ephemeral.secretKey, decodeBase64Url(challenge.hostEphemeralPublicKey), false).slice(1, 33);
    return {
...
export function verifyRelayHostAcceptance(pending: PendingRelaySession, value: unknown): boolean {
  if (!isRecord(value) || value.type !== "session.key.accepted" || Object.keys(value).length !== 2 ||
      typeof value.signature !== "string" || !/^[A-Za-z0-9_-]{86}$/u.test(value.signature)) {return false;}
  try {
    return p256.verify(decodeBase64Url(value.signature), pending.transcript, decodeBase64Url(pending.hostIdentityPublicKey), { lowS: false });
  } catch {
    return false;
  }
}
...
export class RelayEncryptedChannel {
  ...
  static create(secret: Uint8Array, transcript: Uint8Array): RelayEncryptedChannel {
    ...
    return new RelayEncryptedChannel(
      material.slice(32, 64), material.slice(0, 32), material.slice(68, 72), material.slice(64, 68), transcriptHash, 2, 1);
  }

  static createHostForConformance(secret: Uint8Array, transcript: Uint8Array): RelayEncryptedChannel {
    const transcriptHash = sha256(transcript);
    const prk = hmac(sha256, transcriptHash, secret);
    const material = expand(prk, encoder.encode("voltura-air-relay-session-keys-v1"), 72);
    return new RelayEncryptedChannel(
      material.slice(0, 32), material.slice(32, 64), material.slice(64, 68), material.slice(68, 72), transcriptHash, 1, 2);
  }
  ...
  private async runSendQueue(): Promise<void> {
    try {
      ...
      this.sending = false;
      if (this.sendQueue.length > 0) {this.drainSendQueue();}
    }
  }

  decryptText(value: unknown): Promise<string | null> {
    if (!(value instanceof ArrayBuffer)) {return Promise.resolve(null);}
  ...
  private async decryptFrame(frame: Uint8Array): Promise<string | null> {
    if (frame.length < 26 || frame[0] !== 1 || frame[1] !== this.receiveDirection) {return null;}
    const counter = readCounter(frame.subarray(2, 10));
    if (counter !== this.receiveCounter + 1n) {return null;}
    try {
      const key = await crypto.subtle.importKey("raw", copyBuffer(this.receiveKey), "AES-GCM", false, ["decrypt"]);
      const plaintext = await crypto.subtle.decrypt(
        { name: "AES-GCM", iv: copyBuffer(nonce(this.receiveNoncePrefix, counter)), additionalData: copyBuffer(concat(this.transcriptHash, frame.subarray(0, 10))), tagLength: 128 },
        key,
  ...
  private async encrypt(plaintext: Uint8Array): Promise<Uint8Array> {
    ...
    const key = await crypto.subtle.importKey("raw", copyBuffer(this.sendKey), "AES-GCM", false, ["encrypt"]);
    const encrypted = new Uint8Array(await crypto.subtle.encrypt(
      { name: "AES-GCM", iv: copyBuffer(nonce(this.sendNoncePrefix, counter)), additionalData: copyBuffer(concat(this.transcriptHash, header)), tagLength: 128 },
      key,
...
function mergeRelativeMovement(currentText: string, nextText: string): string | null {
  try {
    ...
    if (!Number.isSafeInteger(dx) || !Number.isSafeInteger(dy) ||
        Math.abs(dx) > maximumPointerDelta || Math.abs(dy) > maximumPointerDelta) {
      return null;
    }
...
function writeCounter(target: Uint8Array, value: bigint): void { new DataView(target.buffer, target.byteOffset, 8).setBigUint64(0, value); }
function readCounter(value: Uint8Array): bigint { return new DataView(value.buffer, value.byteOffset, 8).getBigUint64(0); }
...
function copyBuffer(value: Uint8Array): ArrayBuffer { return Uint8Array.from(value).buffer; }
