...
export async function connectSecureDirect(routeId: string, signal?: AbortSignal): Promise<{ channel: RTCDataChannel; cleanup: () => void }> {
  if (!/^[A-Za-z0-9_-]{22}$/u.test(routeId)) {throw new TypeError("Invalid Secure Direct route.");}
  ...
  const cancelled = new Promise<never>((_, reject) => { cancelSetup = reject; });
  const onAbort = () => {
    cleanup();
  ...
  try {
    ...
    const offer = await Promise.race([new Promise<string>((resolve, reject) => {
      signaling.addEventListener("message", (event) => {
        if (typeof event.data !== "string" || new TextEncoder().encode(event.data).length > maximumSignalingBytes) {
          reject(new Error("Secure Direct offer was invalid."));
          return;
        }
        try {
          const value: unknown = JSON.parse(event.data);
          if (!isDescription(value, "secure.offer")) {throw new Error();}
          resolve(value.sdp);
        } catch { reject(new Error("Secure Direct offer was invalid.")); }
      }, { once: true });
      signaling.addEventListener("close", () => { if (!answerSent) {reject(new Error("Secure Direct signaling closed."));} }, { once: true });
      signaling.addEventListener("error", () => { if (!answerSent) {reject(new Error("Secure Direct signaling failed."));} }, { once: true });
    }), cancelled]);
    ...
    const channelPromise = new Promise<RTCDataChannel>((resolve, reject) => {
      peer.addEventListener("datachannel", (event) => {
        ...
        if (channel.readyState === "open") {resolve(channel);}
          ...
          const onOpen = () => { removeSetupListeners(); resolve(channel!); };
          const onClose = () => { removeSetupListeners(); reject(new Error("Secure Direct DataChannel closed.")); };
          const onError = () => { removeSetupListeners(); reject(new Error("Secure Direct DataChannel failed.")); };
    ...
    if (!sdp || new TextEncoder().encode(sdp).length > maximumSdpBytes || signaling.readyState !== WebSocket.OPEN) {
      throw new Error("Secure Direct answer was unavailable.");
    }
    const answer = JSON.stringify({ type: "secure.answer", sdp });
    if (new TextEncoder().encode(answer).length > maximumSignalingBytes) {throw new Error("Secure Direct answer was too large.");}
    answerSent = true;
...
function isDescription(value: unknown, type: "secure.offer"): value is { type: "secure.offer"; sdp: string } {
  return typeof value === "object" && value !== null && !Array.isArray(value) &&
    Object.keys(value).length === 2 && (value as { type?: unknown }).type === type &&
