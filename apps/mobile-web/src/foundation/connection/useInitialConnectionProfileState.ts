...
import { hasPairingTokenParameter, parseHostedConnectionAddress, parsePairingLink, parsePcUrl } from "../pairing/pairingLink";
import {
  createPcProfile,
...
export function useInitialConnectionProfileState() {
  ...
  const initialPairing = useMemo(() => hostedAddress?.pairToken
    ? { pairToken: hostedAddress.pairToken, pcUrl: hostedAddress.pcUrl }
    : parsePairingLink(window.location.href), [hostedAddress]);
  const addressHasInvalidPairing = useMemo(
    () => initialPairing === null && hasPairingTokenParameter(window.location.href),
    [initialPairing]
  );
  const addressHasPcUrl = useMemo(() => hostedAddress !== null || hasPcUrlInAddress(window.location.href), [hostedAddress]);
  ...
  const hostedAddressIsStored = hostedAddress === null || storedPcProfiles.some((profile) =>
    profile.id === addressPcProfile.id && profile.url === addressPcProfile.url);
  ...
  return {
    ...
    initialPairing,
    screenshotMode,
    shouldActivateAddressPc: !addressHasInvalidPairing && initialPairing === null && hostedAddressIsStored && (addressHasPcUrl || hasOnlyAddressIdentity),
    shouldStoreAddressPc: !addressHasInvalidPairing && initialPairing === null && hostedAddressIsStored && (addressHasPcUrl || hasOnlyAddressIdentity),
    storedPcProfiles
  };
}
