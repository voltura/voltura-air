...
export function useVolturaAirConnection() {
  ...
  const [activePcId, setActivePcId] = useState<string | null>(() =>
    initialPairing !== null
      ? null
      : shouldActivateAddressPc
        ? addressPcProfile.id
  ...
  const activePc = useMemo(
    () => pairedPcs.find((pc) => pc.id === activePcId) ?? null,
    [activePcId, pairedPcs],
  );
  ...
  const [state, setState] = useState<ConnectionState>(() =>
    connectionPc ? "connecting" : "needs-pairing",
  );
  const [message, setMessage] = useState(() =>
    connectionPc
      ? "Connecting to PC..."
      : hasStoredPcWithoutConnection
        ? "Choose a PC or scan a pairing QR."
        : "Scan the PC pairing QR to pair this app.",
  );
  ...
  const {
    ...
    requestUrlOpen,
    urlOpenResult,
  } = useUrlOpen(state, send);
  const {
    completeTextTransfer: completeTextTransferState,
    pendingTextTransfer,
    requestTextTransfer,
    textTransferResult,
  } = useTextTransfer(state, send);
  const {
  ...
  } = useDiagnostics(state, connectionEpoch, send);
  ...
  const displayMessage =
    state === "paired" && activePc && message.startsWith("Connected to ")
  ...
  useConnectionSocketLifecycle({
    ...
    connectionPcUrl,
    deviceNameRef,
    getLatestConnectionPc: (fallback) =>
      connectionPc?.id === fallback.id && connectionPc.url === fallback.url
        ? connectionPc
        : fallback,
    lastHealthyAtRef,
    lastUserActivityAtRef,
