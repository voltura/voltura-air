...
describe("browserStorage", () => {
  it("retains a volatile write when reads work but persistent writes are quota-blocked", () => {
    vi.stubGlobal("localStorage", storageWith({ getItem: () => null, setItem: () => { throw new DOMException("Full", "QuotaExceededError"); } }));
  ...
  it("retains a volatile removal when persistent removal is blocked", () => {
    vi.stubGlobal("localStorage", storageWith({ getItem: () => "stale", removeItem: () => { throw new DOMException("Blocked", "SecurityError"); } }));
