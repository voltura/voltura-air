...
export function hashSessionDescription(sdp: string): string {
  return encodeBase64Url(sha256(textEncoder.encode(sdp)));
}

export function verifyHostSessionSignature(publicKey: string, signature: string, transcript: string): boolean {
  try {
