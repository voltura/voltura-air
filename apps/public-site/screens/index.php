...
try {
    ...
    $items = $stmt->fetchAll();
    $sortLabels = ['popular' => 'Most popular', 'newest' => 'Newest', 'author' => 'Author A-Z'];
    $sortOptions = '';
    foreach ($sortLabels as $value => $label) {
        $selected = $sort === $value;
        $sortOptions .= '<button type="button" role="option" class="catalog-sort-option" data-sort-value="' . air_screen_h($value) . '" aria-selected="' . ($selected ? 'true' : 'false') . '">' . air_screen_h($label) . '</button>';
    }
    $body = (isset($_GET['deleted']) ? air_screen_toast('Custom screen deleted') : '') . '<p class="catalog-lede">Browse custom screens shared by the Voltura Air community.</p><form class="catalog-search" method="get"><label class="sr-only" for="catalog-query">Search screens</label><span class="catalog-query" data-catalog-query><input id="catalog-query" name="q" value="' . air_screen_h($query) . '" placeholder="Search by name, author, description, or tag" maxlength="100"><button type="button" class="catalog-tag-remove catalog-query-clear" aria-label="Clear search" hidden></button></span><label class="sr-only" for="catalog-sort">Sort screens</label><div class="catalog-sort" data-catalog-sort><select id="catalog-sort" name="sort"><option value="popular"' . ($sort === 'popular' ? ' selected' : '') . '>Most popular</option><option value="newest"' . ($sort === 'newest' ? ' selected' : '') . '>Newest</option><option value="author"' . ($sort === 'author' ? ' selected' : '') . '>Author A-Z</option></select><button type="button" class="catalog-sort-trigger" aria-haspopup="listbox" aria-expanded="false" aria-controls="catalog-sort-options"><span>' . air_screen_h($sortLabels[$sort]) . '</span><span class="catalog-sort-chevron" aria-hidden="true"></span></button><div id="catalog-sort-options" class="catalog-sort-options" role="listbox" tabindex="-1" hidden>' . $sortOptions . '</div></div><button type="submit">Search</button></form><section class="feature-band catalog-grid">';
    foreach ($items as $item) {
        ...
        $adminDelete = $isAdmin
            ...
            : '';
        $body .= '<article>' . air_screen_preview((string)$item['screen_json'], (string)$item['name'], true) . '<div class="catalog-card-copy"><h2><a href="' . $detailUrl . '">' . air_screen_h($item['name']) . '</a></h2>' . air_screen_stars((float)$item['average_rating'], (int)$item['rating_count']) . '<p>' . air_screen_h($item['description']) . '</p><small>By ' . air_screen_h($item['author']) . ' &middot; ' . air_screen_h($date) . '<br><span class="catalog-card-tags">' . air_screen_tag_pills((string)$item['tags']) . '<span>&middot; ' . (int)$item['downloads'] . ' downloads</span></span></small><div class="catalog-card-actions"><a class="button secondary catalog-preview-link" href="' . $detailUrl . '">Preview</a>' . $adminDelete . '</div></div></article>';
    }
    if (!$items) {
