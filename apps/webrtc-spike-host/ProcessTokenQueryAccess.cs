...
internal static partial class ProcessTokenQueryAccess
{
    ...
    internal static void Grant(SecurityIdentifier serviceSid)
    {
        ...
        Grant(process, serviceSid, ProcessQueryLimitedInformation, "spike host process");

        if (!OpenProcessToken(process, TokenQuery | ReadControl | WriteDac, out nint token))
            throw new InvalidOperationException($"Could not open the spike host token security descriptor ({Marshal.GetLastPInvokeError()}).");
        try
        {
            Grant(token, serviceSid, TokenQuery, "spike host token");
        }
        finally
        {
            CloseHandle(token);
    ...
    private static void Grant(nint handle, SecurityIdentifier serviceSid, int accessMask, string objectName)
    {
        ...
        if (required == 0) throw new InvalidOperationException("The spike host process security descriptor is unavailable.");
