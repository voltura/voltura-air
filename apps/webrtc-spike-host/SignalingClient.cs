...
internal sealed class SignalingClient : IDisposable
{
    ...
    internal async Task CreateOfferAsync(string room, EncryptedEnvelope payload, CancellationToken cancellationToken)
    {
        SignalResponse response = await PostAsync(new { op = "create", room, payload }, cancellationToken).ConfigureAwait(false);
        EnsureOk(response, "publish the offer");
    }

    internal async Task<EncryptedEnvelope> WaitForAnswerAsync(
        string room,
        TimeSpan timeout,
        CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            if (response.Ready && response.Payload is not null) return response.Payload;
    ...
    private async Task<SignalResponse> PostAsync<T>(T payload, CancellationToken cancellationToken)
    {
        ...
        {
            string contentType = response.Content.Headers.ContentType?.ToString() ?? "missing Content-Type";
            throw new InvalidOperationException(
                $"Signaling returned HTTP {(int)response.StatusCode} with invalid JSON " +
                $"({contentType}; {body.Length} characters). Verify that the URL serves the spike signal.php file.",
                exception);
        }
        if (result is null) throw new InvalidOperationException($"Signaling returned HTTP {(int)response.StatusCode} with an empty response.");
        if (!response.IsSuccessStatusCode && result.Ok) throw new InvalidOperationException($"Signaling returned HTTP {(int)response.StatusCode}.");
        return result;
    }
    ...
    private sealed record SignalResponse(bool Ok, bool Ready = false, EncryptedEnvelope? Payload = null, string? Error = null);
}
