...
{
    internal const QRCodeGenerator.ECCLevel ErrorCorrectionLevel = QRCodeGenerator.ECCLevel.M;

    public static BitmapSource Create(string url, bool includeIcon = true)
    {
        var iconPath = Path.Combine(AppContext.BaseDirectory, "Assets", "VolturaAir-256.png");
        using var icon = includeIcon && File.Exists(iconPath) ? new System.Drawing.Bitmap(iconPath) : null;
        using var generator = new QRCodeGenerator();
        using var data = generator.CreateQrCode(url, ErrorCorrectionLevel);
