...
public partial class MainWindow : Window
{
    ...
    private UIElement BuildDiagnosticsPage()
    {
        ...
        var actions = new StackPanel { Orientation = Orientation.Horizontal, Margin = new Thickness(0, 16, 0, 0) };
        actions.Children.Add(CreateButton("Copy all", (_, _) => System.Windows.Clipboard.SetText(string.Join(Environment.NewLine, details.Select(detail => $"{detail.Name}: {detail.Value}"))), primary: true));
    ...
    private IReadOnlyList<DiagnosticItem> GetDiagnostics()
    {
        return
        [
            new("Version", AppVersion.Display),
            new("Host URL", _serverUrl),
            new("Advertised IP", _webHost.AdvertisedHostAddress),
            new("Pairing link", _pairingUrl),
