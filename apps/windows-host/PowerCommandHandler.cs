...
{
    public async Task HandleAsync(WebSocket socket, string clientId, string action, CancellationToken cancellationToken)
    {
        if (!SystemPowerActions.IsSupported(action))
        {
            LogAction(clientId, action, "unsupported");
            await SendResultAsync(socket, clientId, action, false, "VAIR-POWER-UNSUPPORTED", "This power action is not supported.", cancellationToken);
            return;
        }
        ...
        if (!powerController.IsActionAvailable(action))
        {
            ...
            await SendResultAsync(socket, clientId, action, false, "VAIR-POWER-UNAVAILABLE", "This Windows feature is not available on the PC.", cancellationToken);
            return;
        }
        ...
        if (!CanExecute(clientId, action))
        {
            ...
            await SendResultAsync(socket, clientId, action, false, "VAIR-POWER-DENIED", "This action is disabled by the PC host.", cancellationToken);
            return;
        }
        ...
        if (action == SystemPowerActions.Lock)
        {
            ...
            if (lockStatus.State == WorkstationLockPolicyState.Disabled)
            {
                ...
                await SendResultAsync(socket, clientId, action, false, "VAIR-POWER-LOCK-DISABLED", "Windows locking is disabled. Enable it in the Voltura Air host settings.", cancellationToken);
                return;
            }
            ...
            if (lockStatus.State == WorkstationLockPolicyState.Unavailable)
            {
                ...
                await SendResultAsync(socket, clientId, action, false, "VAIR-POWER-LOCK-UNAVAILABLE", "Windows rejected the lock request. Open the Voltura Air host settings, enable Windows locking, and try again.", cancellationToken);
                return;
            }
        }
        ...
        catch (Exception ex) when (ex is not WebSocketException and not OperationCanceledException and not ObjectDisposedException)
        {
            ...
            await SendResultAsync(socket, clientId, action, false, "VAIR-POWER-EXECUTION-FAILED", GetFailureMessage(action), cancellationToken);
            return;
        }
        ...
        await SendResultAsync(socket, clientId, action, true, null, GetSuccessMessage(action), cancellationToken);
    }
    ...
    {
        await transport.SendAsync(socket, new { type = "system.power.result", action, succeeded, code, message }, cancellationToken);
