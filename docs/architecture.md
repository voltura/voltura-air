...
| Apps discovery and previews                 | The command handler owns connection-scoped revision/opaque-ID maps and at most one host-wide short-lived preview session. The Windows adapter performs bounded current-session/current-desktop discovery, action revalidation, and one in-memory capture at a time. Mobile owns the Apps peer, binary assembly buffers, object URLs, carousel listeners, shared input-foundation momentum scroller, and delayed launch refresh. Closing, disconnect, permission loss, replacement, failure, or shutdown releases the complete preview chain; no worker, timer, discovery, capture, or peer exists while Apps is closed.                                          |
...
metadata and fresh opaque IDs only. Static JPEG previews use a distinct
