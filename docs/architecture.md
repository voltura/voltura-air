...
| Cursor and watchdog | Custom pointer is one host-wide Windows scheme. The host restores the configured scheme on disable/exit; a single ready-confirmed watchdog restores it after abnormal host exit. Replacement hosts wait for an earlier watchdog to finish restoration. |
