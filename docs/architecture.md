...
| Opaque open-window discovery, action revalidation, and short-lived static previews                                              | `AppsCommandHandler`, `WindowsAppsWindowAdapter`, `AppsPreviewSessionCoordinator`, `AppsWindowPreviewCapture`, and mobile `features/apps`                                                      |
...
| Apps discovery and previews                 | The command handler owns connection-scoped revision/opaque-ID maps and at most one host-wide short-lived preview session. The Windows adapter performs bounded current-session/current-desktop discovery, native-identity and current-permission action revalidation, and one in-memory capture at a time. The Apps preview peer coalesces its bounded inbound request slot to the newest visible-card request. Mobile owns the Apps peer, binary assembly buffers, object URLs, carousel listeners, shared input-foundation momentum scroller, and explicit entry/action/background-return refreshes. Closing, disconnect, permission loss, replacement, failure, or shutdown releases the complete preview chain; no worker, timer, discovery, capture, or peer exists while Apps is closed. |
...
lifecycle. The shell loads `features/apps` only on entry. `AppsCommandHandler`
owns bounded connection-scoped window maps; `WindowsAppsWindowAdapter`
enumerates and revalidates only current-session, current-virtual-desktop
application windows. `AppsPreviewSessionCoordinator` owns exactly one optional
preview peer, while `AppsWindowPreviewCapture` owns bounded static capture and
encoding after adapter revalidation. JSON carries bounded display
metadata and opaque IDs only. A verified unchanged native identity may retain its
opaque ID across list revisions. A higher-integrity window that UIPI prevents the
host from lifetime-tagging remains listable, but preview and close fail closed.
Static JPEG previews use a distinct
data-channel-only peer created through the lower-level reliable ordered
`FileTransferWebRtcPeer` factory, with shared ICE/TURN configuration and TLS TURN
bridge primitives but no `FileTransferCoordinator`, file state, Screen peer, or
Phone webcam owner. Mobile requests only the centered card and neighbors, owns
all assembly buffers and object URLs, and tears them down on every exit or
authorization/transport boundary.
...
Files follows the same lazy feature boundary: the initial shell carries only capability/navigation wiring and loads `features/file-manager` on entry. The host resolves every opaque session, location, entry, revision, continuation, and job reference. Effective device Files policy is applied while the host constructs each directory revision, including removal of protected Hidden+System items before any client-visible count or opaque entry reference exists. Journal-owned temporary and backup paths are always removed before counts, signatures, or opaque references are built, independently of the protected-item policy. `FileManagerService` validates source and destination revisions together with queue admission so a changed directory cannot redirect or partially resolve an operation. Its bounded workers own the single mutation queue, upload commit/recovery, panel-location persistence, and atomic local job journal; shutdown cancels and awaits them. Every temporary, backup, or partially committed artifact remains durably owned until cleanup, commit, or rollback is confirmed. Windows clipboard, Shell, location-store, and journal behavior remain replaceable adapters so destructive transitions can be fault-injected independently.

`FileTransferCoordinator` is a sibling session owner, not part of `FileManagerService` or a media peer. It owns one host-wide transfer slot, signed offer/answer setup, progress/stall/expiry cancellation, and a dedicated data-channel-only `FileTransferWebRtcPeer`. The peer reuses existing ICE configuration, Relay routing credentials, and the bounded TLS TURN bridge without sharing or genericizing Screen/Phone peers. Downloads hold a stable read handle; uploads enter `FileManagerService` through one internal receiver and therefore reuse its mutation queue, journal, conflicts, and recovery. Upload admission captures the destination directory and signature; execution revalidates that captured directory rather than the panel's later revision or location. The mobile Files sibling owns one picker input, transfer peer, and Save/Share activation. A shared device-file helper owns the existing page-scoped OPFS directory and holds its origin-scoped browser lock for the page lifetime without polling. Startup reuses and sweeps an inherited owner only after acquiring that exclusive claim; a cloned live tab or unavailable claim receives a fresh owner and never sweeps the inherited directory. The helper supports both transferred files and locally generated Screen View recordings without either flow sweeping the other's active artifact.

The Screen View feature owns its browser-local recording controller separately from the live WebRTC workspace. It clones only the received video track and, when locally enabled at start, its audio track; MediaRecorder writes bounded one-second chunks serially into one OPFS partial. The controller owns format selection, duration and byte limits, finalization, Save/Share, and cleanup without adding host, protocol, Relay, permission, upload, or recovery state. It never stops the original live tracks, and recording failure leaves the mirror running.
