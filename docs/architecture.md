...
- `ClientMessageValidator.cs` decodes each bounded authenticated input message once into a `ValidatedInputCommand`; `InputDispatcher.cs` consumes that command without rereading JSON and translates it for `SendInputInjector.cs`. The injector serializes native calls, reuses its single-input movement buffer, and sends text in bounded batches with partial-send cleanup. `PresentationCommands.cs` owns the fixed target-specific presenter shortcut allowlist. `PresentationLaserPointerController.cs` owns transient desired state and device ownership; it delegates native application to the existing cursor service and never persists laser state. Presentation report wire validation, byte-bounded stored-report reads, nested stored-report validation, normalization, and atomic persistence are owned by the focused protocol, validator, report types, and store. Presentation email sharing owns exact attachment fulfillment and its host-owned periodic draft-artifact cleanup. `TextDestination.cs` owns safe focused/clipboard/configured text delivery. `WindowsProcessIntegrity.cs` and `PointerHighlightForegroundMonitor.cs` use a single foreground event hook solely to block remote input when a higher-integrity app owns the foreground; they have no cursor responsibilities.
...
- `CustomPointerService.cs` builds Windows cursors from the packaged maximum-size templates, preserves the Windows role mapping, applies them through the Windows cursor API, and reloads the user's configured cursor scheme on disable or normal exit. `AppPointerSettings.cs` owns one write-through in-memory snapshot for the persisted default speed, custom-pointer appearance, Presentation laser size/color, and default-on recovery-watchdog setting, so status and command paths do not reread the Registry. The cursor service also generates the temporary Presentation laser cursor, applies that one cursor to every role, caches both configured appearances while laser mode overrides the normal scheme, and restores the correct configured scheme without a Registry read on the laser command path.
- `CursorWatchdogService.cs` owns the native monitor while Custom pointer and **Use cursor recovery watchdog** are both enabled. Before starting a monitor, a new host waits for every pre-existing watchdog in its Windows session to finish restoration and exit, with a bounded startup failure instead of applying a cursor concurrently. `VolturaAir.CursorWatchdog.exe` starts a ready-confirmed monitor outside the live host process tree. One monitor is allowed per host process ID. The monitor blocks on a synchronized host-process handle, restores the configured Windows cursor scheme after every confirmed host exit, optionally signals successful restoration for validation, and exits. The host confirms readiness before replacing a cursor. Feature-disable paths restore cursors before explicitly stopping the monitor. Final host shutdown restores cursors, detaches the managed process wrapper without killing the monitor, and lets the monitor perform the harmless second restoration after actual host exit.
