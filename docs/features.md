...
- One transfer runs host-wide beside existing screen/webcam peers. Transfers show bounded progress and Cancel; 60 seconds without committed progress cancels them, and retry starts from zero. Downloads use temporary browser storage that is swept when Files starts or exits, on reload/disconnect/permission loss, and after save. Uploads use the existing mutation queue and Operation Center, cannot pause/resume, validate available PC volume space and Windows names, and resolve conflicts with Replace, Keep both, or Cancel. Browser quota and PC free-space checks reject early when available; actual I/O remains authoritative.
