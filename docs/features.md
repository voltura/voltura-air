...
- A compact **Transfer** menu handles one file at a time. **Save to this device** is available for exactly one explicitly selected PC file when the browser can write OPFS; after transfer it exposes a fresh **Save to Files / Share** action, with browser download fallback and explicit discard. Canceling the native share sheet leaves that action ready to retry while Files remains open; a completed handoff frees the transfer slot. **Choose file from this device** remains available without OPFS and uploads into the active PC folder; completion refreshes the destination and selects the host-confirmed row when it is within the bounded loaded pages. There are no device folders, batches, synchronization, resume, or transfer history.
