...
| Phone microphone or camera as a Windows input | Keep microphone and webcam modes separate. The isolated iPhone-webcam spike proved 1920×1080/30 browser capture and live H.264 rendering over both Direct LAN and existing relay-only TURN through a current-user Windows virtual camera into VLC, Chrome, and Teams. Teams passed sequentially after Chrome released the camera; simultaneous multi-consumer use was not proved. Every camera exposed by Chrome on the tested iPhone 17 Pro Max switched live on the original Relay peer. Switching from Chrome to Messenger caused iOS to close that peer; the spike correctly stopped capture and retained the host, but could not resume. Product integration must treat foreground return as a fresh-session boundary and reconnect automatically with a new peer and current Relay credentials. The working path explicitly controls the actual WebRTC sender resolution/bitrate policy, retains H.264 decoder configuration across recovery, requests key frames after loss, drops stale work through capacity-one latest-frame ownership, and paces the virtual camera at 30 fps. Edge compatibility and the numeric ≤300 ms gate remain unproven. Use WebRTC media tracks rather than expanding the control DataChannel. |
| PC Screen View quality | Evaluate whether the webcam spike's proven media-path lessons improve the existing Screen View before adding another transport or protocol: compare actual encoded dimensions and frame rate with the captured source, make encoder resolution/bitrate intent explicit, retain capacity-one latest-frame ownership, and pace consumer delivery so load cannot become backlog latency. Preserve the existing Direct/relay-only ICE, 4/2 Mbps quota-derived Relay limits, permissions, and renewal lifecycle. Require real Direct and Relay visual evidence before changing production defaults. |
