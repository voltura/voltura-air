...
- **Remote** supports media playback, press-and-hold seek, video fullscreen, browser fullscreen, volume, mute, Standard/YouTube/Kodi shortcut modes, Kodi video/UI toggle, fullscreen/windowed toggle, and stop/info/subtitle/power-menu shortcuts, a default navigation ring with repeatable directional zones and a mini-trackpad center that sends Enter in Kodi mode, optional legacy D-pad with OK, and an Fn panel with Windows window actions and browser tab/page shortcuts. Tap **Switch app** to return to the previous app, or hold it until the visual Windows switcher appears, slide left or right, and release to open the selected app. **Task view** opens the persistent Windows window overview. In phone landscape, the Fn panel replaces the media column and keeps volume and navigation available beside it.
