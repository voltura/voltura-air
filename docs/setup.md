...
- **Send text to PC** lets you compose or paste text, review the destination warning, choose whether to append Enter, and save local snippets. The current release sends to the Windows application that has focus when you press Send; select the correct field on the PC first and do not change focus during delivery.
