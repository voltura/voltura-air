...
Files uses one panel below 640 CSS pixels and two equal panels at or above that content boundary. The app header and compact selector stay outside the Files viewport; action rows and panel headers stay outside each virtualized list scroller. A feature-owned container query controls panel composition. File lists use native contained touch scrolling, including two-finger scrolling, and the feature does not magnify or transform its workspace. Name, Size, Type, and Modified rows share the header grid; names wrap to two lines before truncating, Modified displays date and time on separate lines, and any compact attribute codes are defined by the panel legend. The `..` navigation entry uses the same flat row treatment as folders while leaving its metadata cells empty. Long press opens Properties for a file, folder, or the current location without taking ownership from a scrolling gesture. Locations and Clipboard are transient menus: touching any other Files control or row dismisses them. The persistent Files footer uses an icon, readable type, and neutral/success/error surface tone instead of debug-sized text. Transient operation errors remain visible for eight seconds and can be dismissed explicitly; transient success feedback remains for four seconds.
