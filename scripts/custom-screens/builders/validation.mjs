...
export function validateDefinition(definition) {
  ...
  if (
    !screen ||
    !ids.test(screen.id) ||
    !ids.test(screen.revision) ||
    screen.name.length > 24 ||
    screen.assignedClientIds.length !== 0
  ) {
  ...
  for (const section of screen.sections) {
    if (
      !ids.test(section.id) ||
      seen.has(section.id) ||
      ![3, 4, 6, 8, 9, 12].includes(section.widthColumns)
    ) {
...
function validateAction(action, screenId, buttonId) {
  ...
  if (
    ...
    (!allowedKeys.has(action.key) || !Array.isArray(action.modifiers))
  ) {
  ...
  if (action.kind === "urlOpen") {
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:")
      throw new Error(`Unsupported URL on ${screenId}/${buttonId}.`);
  }
  if (action.kind === "knownApp" && !knownApps.has(action.actionId))
    throw new Error(`Unknown app on ${screenId}/${buttonId}.`);
  if (action.kind === "hostAction" && !hostActions.has(action.actionId))
    throw new Error(`Unknown host action on ${screenId}/${buttonId}.`);
}
