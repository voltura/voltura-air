export async function verifyResponsivePresentationLayout(page) {
  ...
  if (
    (await page.getByRole("button", { name: "Start slideshow", exact: true }).count()) !== 0 ||
  ...
  if (
    (await page.getByRole("button", { name: "Blackout", exact: true }).count()) !== 0 ||
    (await page.getByRole("button", { name: "Laser pointer", exact: true }).count()) !== 1 ||
  ...
  for (const viewport of viewports) {
    ...
    const result = await page.evaluate(
      ({ expectedExit }) => {
        ...
        const firstSecondaryAction = document.querySelector(
          ".presentation-control-secondary button",
        );
        ...
        const timerButtons = Array.from(
          document.querySelectorAll(".presentation-timer-actions button"),
        );
        ...
        const exitElement =
          expectedExit === "compact" ? compactExit : expectedExit === "top" ? topExit : bottomExit;
        return {
          columns: getComputedStyle(surface).gridTemplateColumns.split(" ").filter(Boolean).length,
          exitVisible:
            exitElement instanceof HTMLElement && getComputedStyle(exitElement).display !== "none",
          horizontalOverflow:
          ...
          minTimerActionHeight: Math.min(
            ...timerButtons.map((button) => button.getBoundingClientRect().height),
          ),
          outsideViewportWidth:
            surfaceBounds.left < -1 || surfaceBounds.right > window.innerWidth + 1,
          shortLandscapeHeadingVisible: shortLandscape && (headingBounds?.width ?? 0) > 1,
          shortLandscapeLeftInset:
            shortLandscape && primaryBounds ? Math.abs(primaryBounds.top - surfaceBounds.top) : 0,
    ...
    if (
      ...
      result.minTimerActionHeight < 44 ||
      result.outsideViewportWidth ||
      result.shortLandscapeHeadingVisible ||
      result.shortLandscapeLeftInset > 1 ||
      result.shortLandscapeColumnMisalignment > 1 ||
      result.shortLandscapeTargetVisible ||
      result.targetHiddenOutsideShortLandscape ||
      !result.firstSecondaryActionVisible
    ) {
      throw new Error(
        `Responsive Presentation check failed for ${viewport.name}: ${JSON.stringify(result)}`,
      );
    }
  }
  ...
  const expandedLayout = await page.evaluate(() => {
    ...
    const summaryPrevious = document.querySelector(
      ".presentation-trackpad-summary button:first-of-type",
    );
    const summaryNext = document.querySelector(
      ".presentation-trackpad-summary button:last-of-type",
    );
    if (
      !(surface instanceof HTMLElement) ||
      !(controls instanceof HTMLElement) ||
      !(sideStack instanceof HTMLElement) ||
      !(summaryDetails instanceof HTMLElement) ||
      !(summaryPrevious instanceof HTMLButtonElement) ||
      !(summaryNext instanceof HTMLButtonElement)
    ) {
    ...
    return {
      ...
      summaryOverlapsNavigation:
        detailsBounds.bottom > previousBounds.top + 1 || detailsBounds.bottom > nextBounds.top + 1,
  ...
  if (
    "error" in expandedLayout ||
    expandedLayout.halfWidthDifference > 1 ||
    expandedLayout.leftOutsideSurface > 1 ||
    expandedLayout.rightOutsideSurface > 1 ||
    expandedLayout.summaryOverlapsNavigation ||
    expandedLayout.navigationRowsMisaligned > 1 ||
    expandedLayout.navigationWidthsDiffer > 1 ||
    expandedLayout.minNavigationHeight < 62 ||
    !expandedLayout.navigationLabelsVisible
  ) {
    throw new Error(
      `Expanded Presentation trackpad layout failed: ${JSON.stringify(expandedLayout)}`,
    );
  }
  ...
  if (
    (await page.locator(".presentation-mode.trackpad-open").count()) !== 0 ||
  ...
  const portraitSummary = await page.evaluate(() => {
    ...
    const previous = document.querySelector(
      ".presentation-trackpad-summary-navigation button:first-child",
    );
    const next = document.querySelector(
      ".presentation-trackpad-summary-navigation button:last-child",
    );
    if (
  ...
  if (
    "error" in portraitSummary ||
    portraitSummary.summaryOverlapsNavigation ||
    ...
    throw new Error(
      `Portrait Presentation trackpad summary failed: ${JSON.stringify(portraitSummary)}`,
    );
  }
  await page.locator(".presentation-trackpad-heading").click();
  ...
  if ((await choosePresentation.count()) === 1) {
    ...
    for (const viewport of [
      ...
      const chooserLayout = await page.evaluate(() => {
        ...
        if (
          !(chooser instanceof HTMLElement) ||
          !(lists instanceof HTMLElement) ||
        ...
        return {
          ...
          horizontalOverflow:
            document.documentElement.scrollWidth > document.documentElement.clientWidth + 1,
          outsideViewport:
            chooserBounds.left < -1 ||
            chooserBounds.right > window.innerWidth + 1 ||
            chooserBounds.top < -1 ||
            chooserBounds.bottom > window.innerHeight + 1,
          regionsOverlap: headerBounds.bottom > footerBounds.top,
          scrollable: lists.scrollHeight >= lists.clientHeight,
        };
      });
      if (
        "error" in chooserLayout ||
        chooserLayout.columns !== viewport.columns ||
        chooserLayout.horizontalOverflow ||
        chooserLayout.outsideViewport ||
        chooserLayout.regionsOverlap ||
        !chooserLayout.scrollable
      ) {
        throw new Error(
          `PowerPoint chooser layout failed for ${viewport.name}: ${JSON.stringify(chooserLayout)}`,
        );
      }
    }
  ...
  await page.setViewportSize({ width: 393, height: 852 });
  await page.getByRole("button", { name: "Open menu", exact: true }).click();
  const settingsCloseAppearance = await page
  ...
  if (
    "error" in closeAlignment ||
    closeAlignment.horizontalOffset > 1 ||
    closeAlignment.verticalOffset > 1 ||
    JSON.stringify(closeAlignment.appearance) !== JSON.stringify(settingsCloseAppearance)
  ) {
    throw new Error(
      `Shared modal close control alignment failed: ${JSON.stringify(closeAlignment)}`,
    );
  }
