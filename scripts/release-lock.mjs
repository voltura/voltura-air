...
export function getReleaseLockPath(repositoryRoot) {
  ...
  if (result.status !== 0)
    throw new Error("Git could not resolve the Voltura Air release lock path.");
  return path.resolve(repositoryRoot, result.stdout.trim());
}

export async function withReleaseLock(
  action,
  { repositoryRoot, lockPath, makeDirectory = mkdir, remove = rmdir } = {},
) {
  ...
  try {
    ...
    if (error.code === "EEXIST")
      throw new Error(`Another Voltura Air release is already running (${resolvedLockPath}).`);
    throw error;
  }
  let result;
  let actionError;
  ...
  try {
    await remove(resolvedLockPath);
  } catch (cleanupError) {
    if (actionError)
      throw new AggregateError(
        [actionError, cleanupError],
        `${actionError.message} Release lock cleanup also failed: ${cleanupError.message}`,
      );
    throw cleanupError;
  }
