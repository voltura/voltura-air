...
if (!/^\d+\.\d+\.\d+$/u.test(version ?? "") || !publishRoot)
  throw new Error("Usage: node scripts/sign-update-manifest.mjs VERSION PUBLISH_ROOT");
const keyPath = process.env.VOLTURA_AIR_UPDATE_SIGNING_KEY_PATH;
if (!keyPath)
  throw new Error("VOLTURA_AIR_UPDATE_SIGNING_KEY_PATH is required to sign update manifests.");
const configuredPassphrase = process.env.VOLTURA_AIR_UPDATE_SIGNING_PASSPHRASE;
async function readPassphrase() {
  if (!process.stdin.isTTY)
    throw new Error(
      "An authorized interactive terminal is required for the update signing passphrase.",
    );
...
const passphrase =
  configuredPassphrase === undefined ? await readPassphrase() : configuredPassphrase;
...
const publicKey = createPublicKey(
  await readFile(
    path.resolve("apps/windows-host/Features/Updates/update-signing-public.pem"),
    "utf8",
  ),
);
if (!verify("sha256", bytes, { key: publicKey, padding: 6, saltLength: 32 }, signature))
  throw new Error("Update manifest does not verify with the host public key.");
