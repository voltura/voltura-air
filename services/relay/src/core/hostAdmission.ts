...
export function isHostAuthenticationExpired(authenticationExpiresAt: number, now: number = Date.now()): boolean {
...
export function nextHostAuthenticationDeadline(
  ...
  const pending = authenticationDeadlines.filter((deadline) => !isHostAuthenticationExpired(deadline, now));
...
export function canClaimHostCandidate(
  ...
  return !hasAuthenticatedHost && isCurrentCandidate && isOpen && !isHostAuthenticationExpired(authenticationExpiresAt, now);
}
