...
export async function processHostAuthentication(
  ...
  if (new TextEncoder().encode(text).length > maximumControlMessageBytes)
  ...
  if (!pending) {
    ...
    if (!hello || (await (await import("./routing")).deriveRouteId(hello.publicKey)) !== routeId)
      return { kind: "rejected" };
  ...
  if (
    !proof ||
    !(await verifyHostProof(pending.publicKey, routeId, pending.challenge, proof.signature))
  )
    return { kind: "rejected" };
  return {
    kind: "accepted",
    response: JSON.stringify({ type: "relay.host.accepted", protocol: 1 }),
  };
}
