...
describe("relay route identity", () => {
  ...
  it("authenticates the persistent host using the shared transcript", async () => {
    ...
    const challenged = await processHostAuthentication(
      routeId,
...
describe("relay envelopes", () => {
  it("preserves text and binary inner-frame kinds", () => {
    ...
    const text = decodeEnvelope(
      encodeEnvelope(session, new TextEncoder().encode("{}"), relayEnvelopeKind.text),
    );
    const binary = decodeEnvelope(
      encodeEnvelope(session, new Uint8Array([1, 2]), relayEnvelopeKind.binary),
    );
    const closeDevice = decodeEnvelope(
      encodeEnvelope(session, new Uint8Array(), relayEnvelopeKind.closeDevice),
    );
  ...
  it("allows only the fixed encryption overhead above a 64 KiB application frame", () => {
    ...
    expect(decodeEnvelope(encodeEnvelope(session, maximumPayload))?.payload).toHaveLength(
      maximumRelayPayloadBytes,
    );
    expect(() => encodeEnvelope(session, new Uint8Array(maximumRelayPayloadBytes + 1))).toThrow(
      RangeError,
    );
  });
});
