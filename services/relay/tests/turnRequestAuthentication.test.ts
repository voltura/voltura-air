...
describe("TURN request authentication lifetime", () => {
  ...
  it("accepts managed-PC clock drift through five minutes", () => {
    expect(isTurnRequestTimestampFresh(String(now - maximumTurnRequestClockSkewMs), now)).toBe(true);
    expect(isTurnRequestTimestampFresh(String(now + maximumTurnRequestClockSkewMs), now)).toBe(true);
  });

  it("rejects timestamps outside the bounded clock-skew window", () => {
    expect(isTurnRequestTimestampFresh(String(now - maximumTurnRequestClockSkewMs - 1), now)).toBe(false);
