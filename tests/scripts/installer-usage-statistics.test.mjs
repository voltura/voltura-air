...
test("interactive unset installs require an explicit usage-statistics choice", () => {
  ...
  assert.match(installer, /ReadRegDWORD \$0 HKCU "\$\{SETTINGS_KEY\}" "\$\{USAGE_CONSENT_VALUE\}"/u);
...
test("the safe choice receives initial focus and choices have equal geometry", () => {
  ...
  assert.match(installer, /SendMessage \$HWNDPARENT \$\{WM_NEXTDLGCTL\} \$UsageConsentDenyButton 1/u);
...
test("consent is persisted only after host health and transaction commit", () => {
  ...
  const staleIdentityRead = primary.indexOf('ReadRegStr $0 HKCU "${SETTINGS_KEY}" "${USAGE_ID_VALUE}"');
  const staleIdentityDelete = primary.indexOf('DeleteRegValue HKCU "${SETTINGS_KEY}" "${USAGE_ID_VALUE}"');
  const consentWrite = primary.indexOf('WriteRegDWORD HKCU "${SETTINGS_KEY}" "${USAGE_CONSENT_VALUE}" $UsageConsentChoice');
  assert.ok(staleIdentityRead >= 0 && staleIdentityDelete > staleIdentityRead && consentWrite > staleIdentityDelete);
  assert.match(primary, /WriteRegDWORD HKCU "\$\{SETTINGS_KEY\}" "\$\{USAGE_CONSENT_VALUE\}" \$UsageConsentChoice[\s\S]*ReadRegDWORD/u);
  assert.equal(primary.match(/WriteRegDWORD/gu)?.length, 1);
  assert.match(persistence, /usage_consent_write_failed:[\s\S]*WriteRegDWORD[^\n]* 2[\s\S]*ReadRegDWORD/u);
  assert.match(persistence, /usage_consent_delete_fallback:[\s\S]*DeleteRegValue[\s\S]*ReadRegDWORD[\s\S]*usage_consent_warn_unknown:/u);
  assert.match(persistence, /usage_consent_warn_unknown:[\s\S]*UsageConsentStateUnknown 1/u);
  assert.match(installer, /UsageConsentStateUnknown == 1[\s\S]*BM_SETCHECK[\s\S]*EnableWindow \$mui\.FinishPage\.Run 0/u);
});
