...
test("installer becomes visible after launch without remaining always on top", () => {
  const restore =
    installer.match(/Function RestoreInstallerWindow(?<body>[\s\S]*?)FunctionEnd/u)?.groups?.body ??
    "";
  const finish =
    installer.match(/Function FinishInstallerWindowActivation(?<body>[\s\S]*?)FunctionEnd/u)?.groups
      ?.body ?? "";
  ...
  assert.match(
    installer,
    /!define MUI_PAGE_CUSTOMFUNCTION_SHOW FinishInstallerWindowActivation\s*!insertmacro MUI_PAGE_WELCOME/u,
  );
