...
const [
  ...
  read(
    "apps/windows-host/Features/PhoneWebcam/Native/VirtualCameraMediaSource/SimpleFrameGenerator.cpp",
  ),
...
test("installer offers the official VB-CABLE page only after confirmed absence", () => {
  const finish =
    installer.match(/Function ConfigureVbCableFinishOption(?<body>[\s\S]*?)FunctionEnd/u)?.groups
      ?.body ?? "";
  ...
  assert.ok(
    installer.indexOf('Section /o "Phone Webcam" SEC_PHONE_WEBCAM') <
      installer.indexOf("Function ConfigureVbCableFinishOption"),
...
test("Windows Modify reuses the existing installer for Phone Webcam on and off", () => {
  const init =
    installer.match(/Function \.onInit(?<body>[\s\S]*?)FunctionEnd/u)?.groups?.body ?? "";
  const finalize =
    installer.match(/Section -FinalizeInstall(?<body>[\s\S]*?)SectionEnd/u)?.groups?.body ?? "";
  const removal =
    installer.match(/Section -ApplyPhoneWebcamRemoval(?<body>[\s\S]*?)SectionEnd/u)?.groups?.body ??
    "";
  const uninstall =
    installer.match(/Section "Uninstall"(?<body>[\s\S]*?)SectionEnd/u)?.groups?.body ?? "";
  assert.match(
    installer,
    /!define MAINTENANCE_INSTALLER "\$LOCALAPPDATA\\Voltura Air\\VolturaAir-Modify\.exe"/u,
  );
  assert.match(init, /Call ConfigureInstalledWebcamSelection/u);
  assert.match(
    installer,
    /Function ConfigureInstalledWebcamSelection[\s\S]*SectionSetFlags \$\{SEC_PHONE_WEBCAM\} \$\{SF_SELECTED\}[\s\S]*SectionSetFlags \$\{SEC_PHONE_WEBCAM\} 0/u,
  );
  assert.match(
    installer,
    /Section -PrepareMaintenanceEntry[\s\S]*Call PrepareMaintenanceInstaller/u,
  );
  assert.match(installer, /CopyFiles \/SILENT "\$EXEPATH" "\$MaintenanceInstallerPending"/u);
  assert.match(
    installer,
    /MoveFileExW\(w "\$MaintenanceInstallerPending", w "\$\{MAINTENANCE_INSTALLER\}", i 9\)/u,
  );
  assert.match(
    finalize,
    /Call PromoteMaintenanceInstaller[\s\S]*WriteRegStr HKCU "\$\{UNINSTALL_KEY\}" "ModifyPath" "\$\\"\$\{MAINTENANCE_INSTALLER\}\$\\""/u,
  );
  assert.match(
    finalize,
    /Call DeletePreparedMaintenanceArtifacts[\s\S]*Call RestorePhoneWebcamAfterFailure[\s\S]*Call RollbackPromotedInstall/u,
  );
  ...
  assert.match(
    removal,
    /SectionGetFlags \$\{SEC_PHONE_WEBCAM\}[\s\S]*cleanup-required[\s\S]*VolturaAir\.WebcamSetup\.exe" remove/u,
  );
  assert.match(
    uninstall,
    /\.removing-main[\s\S]*Rename "\$\{MAINTENANCE_INSTALLER\}" "\$4"[\s\S]*Delete \/REBOOTOK "\$4"/u,
  );
  assert.match(uninstall, /\.removing-main[\s\S]*\.removing-pending[\s\S]*\.removing-rollback/u);
  assert.match(
    uninstall,
    /uninstall_drain_maintenance:[\s\S]*Delete \/REBOOTOK "\$4"[\s\S]*IfFileExists "\$\{MAINTENANCE_INSTALLER\}" uninstall_maintenance_cleanup_failed/u,
  );
  assert.ok(
    uninstall.indexOf("uninstall_maintenance_cleanup_ready:") <
      uninstall.indexOf("-Action CompleteRemoval"),
  );
  assert.ok(
    uninstall.indexOf("-Action CompleteRemoval") <
      uninstall.indexOf('DeleteRegKey HKCU "${UNINSTALL_KEY}"'),
  );
});
...
test("uninstall removes Phone webcam before deleting its recovery helper", () => {
  ...
  assert.ok(
    uninstall.indexOf("Call un.RemovePhoneWebcam") < uninstall.indexOf("-Action StageRemoval"),
  );
  const removal =
    installer.match(/Function un\.RemovePhoneWebcam(?<body>[\s\S]*?)FunctionEnd/u)?.groups?.body ??
    "";
  const uninstallInit =
    installer.match(/Function un\.onInit(?<body>[\s\S]*?)FunctionEnd/u)?.groups?.body ?? "";
  assert.match(
    installer,
    /!define WEBCAM_PROTECTED_SETUP "\$PROGRAMFILES64\\Voltura Air Webcam\\VolturaAir\.WebcamSetup\.exe"/u,
  );
...
test("installer-owned Phone Webcam maintenance restores the prior component and app on failure", () => {
  const install =
    installer.match(/Section \/o "Phone Webcam"(?<body>[\s\S]*?)SectionEnd/u)?.groups?.body ?? "";
  const restore =
    installer.match(/Function RestorePhoneWebcamAfterFailure(?<body>[\s\S]*?)FunctionEnd/u)?.groups
      ?.body ?? "";
  assert.ok(
  ...
  assert.ok(
    install.indexOf("Call RestorePhoneWebcamAfterFailure") <
      install.lastIndexOf("Call RollbackPromotedInstall"),
  );
});
