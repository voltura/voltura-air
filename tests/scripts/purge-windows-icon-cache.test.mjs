...
test("purges Windows icon caches between stopping the host and restarting Explorer", () => {
  ...
  assert.deepEqual(operations, [
    ...
    {
      command: "tasklist",
      args: ["/FI", "IMAGENAME eq explorer.exe", "/FI", "SESSION eq 1", "/FO", "CSV", "/NH"],
    },
...
test("does not launch another Explorer when stopping the current shell fails", () => {
  ...
  assert.equal(
    commands.some(({ command }) => command.endsWith("powershell.exe")),
    false,
  );
});

test("refuses to alter caches outside Windows", () => {
  assert.throws(() => purgeWindowsIconCache({ platform: "linux" }), /only supported on Windows/i);
});
