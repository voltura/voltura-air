...
test("local site development is isolated from production configuration", () => {
  ...
  assert.match(initializer, /Initialize-SetupDatabase/u);
  assert.match(initializer, /Existing local MariaDB administrator password/u);
  assert.match(initializer, /--host=127\.0\.0\.1/u);
  assert.match(initializer, /--protocol=TCP/u);
  const storage = read("scripts/setup/Setup.Storage.psm1");
  assert.match(storage, /RedirectStandardInput = \$true/u);
  assert.match(storage, /ReadToEndAsync/u);
  assert.match(storage, /Never return server error text/u);
