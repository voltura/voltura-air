...
const signing = await readFile(
  new URL("../../scripts/sign-update-manifest.mjs", import.meta.url),
  "utf8",
);
...
test("silent automatic update switch is detected from GetOptions success", () => {
  assert.match(
    installer,
    /ClearErrors\s+\$\{GetOptions\} \$CMDLINE "\/AUTOUPDATE" \$0\s+IfErrors auto_update_missing/u,
  );
...
test("release signing accepts a non-empty environment passphrase without prompting", async () => {
  ...
  await assert.rejects(
    execFileAsync(
      process.execPath,
      ["scripts/sign-update-manifest.mjs", "1.2.3", "artifacts/publish"],
      {
        env: {
          ...process.env,
          VOLTURA_AIR_UPDATE_SIGNING_KEY_PATH: "unused",
          VOLTURA_AIR_UPDATE_SIGNING_PASSPHRASE: " supplied ",
        },
      },
    ),
    ...
    execFileAsync(
      ...
      ["scripts/sign-update-manifest.mjs", "1.2.3", "artifacts/publish"],
      {
        env: {
          ...process.env,
          VOLTURA_AIR_UPDATE_SIGNING_KEY_PATH: "unused",
          VOLTURA_AIR_UPDATE_SIGNING_PASSPHRASE: " \t ",
        },
      },
    ),
